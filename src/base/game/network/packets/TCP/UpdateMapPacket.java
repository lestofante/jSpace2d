...
public class UpdateMapPacket extends TCP_Packet {
	...
	protected boolean mapPacket() {
		...
		int playerNumber = buffer.getChar() & 0xFF; //how many player there are?
		...
		for (int i=0;i<playerNumber;i++){
			...
			if (buffer.remaining() < 2){
				//no number of entity present, add back how many player are left and return underflow error
			...
			//now we know how many entity for this player we have. we have just to check if there are enough byte for this bunch read
			if (buffer.remaining() >= dimensionPlayer+entityNumber*dimensionEntity){
				//we have enough byte to read ONE player and all of its entity! read them all!!
				...
				//no player+entitys data present, add back how many player are left, the number of entity for this player and return underflow error
